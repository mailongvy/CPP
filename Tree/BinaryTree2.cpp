...
// ham trả về node có giá trị liền kề với node đã cho
...
void DeleteNode(tree &t, int data) {
    ...
    else
    {
        ...
        else // data == t->data đã tìm thấy node cần xoá
        {
            // tạo ra một node phụ để sau tạo mối liên kết và xoá
            node* x = t;
            // cây có một con 
            if (t->left == NULL)
            {
                t = t->right; //duyệt sang phải cái node cần xoá để cập nhật mối liên kết giữa node cha của node cần xoá và node con node cần xoá
            }
            else if (t->right == NULL)
            {
                t = t->left; // duyệt sang trái cái node cần xoá để cập nhật mối liên kết giữa node cha của node cần xoá và node con node cần xoá
            }
            else {
...
/*trc khi xoá một node có hai con(con trái và con phải) thì trước hết phải tim 1 node có giá trị liền kề với nó trong cây nhị phân vì
để xoá 1 node có hai con thì phải đảm bảo tính chaast của cây nhị phân*/
